...
def calc(weapon):
    ...
    ms_per_shot = 1000 / (weapon.rpm/60)
    ...
    deg = len(shot_ms) if len(shot_ms) < 30 else 30

    fun_y = np.poly1d(np.polyfit(shot_ms, weapon_y, deg))
    fun_x = np.poly1d(np.polyfit(shot_ms, weapon_x, deg))

    full_vector = []

    for i in range(0, int(weapon.shots * ms_per_shot)):
    ...
    return weapon.name + " = {" + ", ".join(string_vector_parts) + "}"

class Weapon:

    def __init__(self, name, rpm, shots, values):
        self.name = name
        self.rpm = rpm
        self.shots = shots
        self.values = values
